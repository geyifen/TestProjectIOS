...
- (void)createDataViewModel {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSInteger i = 0; i < viewDataArray.count; i++) {
            ...
            if (![subDic.allKeys containsObject:@"dataModel"]) {
                subDic = @{
                    @"dataModel": subDic,
                };
            }
            TestProjectTableModel *tableModel = [TestProjectTableModel yy_modelWithDictionary:subDic].dataModel;
...
- (void)setViewModel:(TestProjectTableModel *)viewModel {
    ...
    [self jumpSelector:viewModel];
}

- (void)jumpSelector:(TestProjectTableModel *)viewModel {
    SEL sel = NSSelectorFromString(viewModel.method);
    @try {
        ((void(*)(id, SEL, TestProjectTableModel *))objc_msgSend)(self, sel, viewModel);
